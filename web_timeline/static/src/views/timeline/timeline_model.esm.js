...
export class TimelineModel extends Model {
    ...
    async load(searchParams) {
        ...
        // Use first field from group_by for ordering
        let field_to_order = this.last_group_bys[0];
        if (this.fields[field_to_order] && this.fields[field_to_order].type === "many2many") {
    ...
    _event_data_transform(record) {
        ...
        let group = -1;
        const parent_group = record[this.last_group_bys[0]];

        if (parent_group && Array.isArray(parent_group) && parent_group.length > 0) {
            if (this.last_group_bys.length > 1) {
                // Nested grouping: use composite ID (parent_id_child_id)
                const child_group = record[this.last_group_bys[1]];
                if (child_group && Array.isArray(child_group) && child_group.length > 0) {
                    group = `${parent_group[0]}_${child_group[0]}`;
                } else {
