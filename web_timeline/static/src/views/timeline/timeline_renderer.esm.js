...
export class TimelineRenderer extends Component {
    ...
    /**
     * Get the groups.
     * Supports nested grouping when multiple group_bys are specified.
     *
     * @param {Object[]} records
     * @private
     * @returns {Array}
    ...
    async split_groups(records) {
        ...
        const nested_field = this.model.last_group_bys.length > 1 ? this.model.last_group_bys[1] : null;

        // Check if there are any unassigned records (no group value)
        const has_unassigned = records.some(
            (evt) => !evt[grouped_field] || !(evt[grouped_field] instanceof Array)
        );

        // Only add UNASSIGNED group if there are actually unassigned records
        if (has_unassigned) {
            groups.push({id: -1, content: _t("<b>UNASSIGNED</b>"), order: -1});
        }

        // For nested grouping, track parent-child relationships
        const parentGroups = new Map(); // parent_id -> {group data, nestedGroups: []}

        var seq = 1;
        for (const evt of records) {
            const group_name = evt[grouped_field];
            if (group_name && group_name instanceof Array) {
                const parent_id = group_name[0];
                const parent_content = group_name[1];

                // Handle nested grouping (2 levels)
                if (nested_field) {
                    const nested_name = evt[nested_field];
                    if (nested_name && nested_name instanceof Array) {
                        const child_id = `${parent_id}_${nested_name[0]}`;
                        const child_content = nested_name[1];

                        // Create or update parent group
                        if (!parentGroups.has(parent_id)) {
                            parentGroups.set(parent_id, {
                                id: parent_id,
                                content: `<b>${parent_content}</b>`,
                                order: seq,
                                nestedGroups: [],
                                showNested: true,
                            });
                        ...
                        // Add child to parent's nestedGroups if not already there
                        const parent = parentGroups.get(parent_id);
                        if (!parent.nestedGroups.includes(child_id)) {
                            parent.nestedGroups.push(child_id);
                            // Add child group
                            groups.push({
                                id: child_id,
                    ...
                    const group = groups.find(
                        (existing_group) => existing_group.id === parent_id
                    );
                    if (group) {
                        continue;
                    }
                    // Check if group is m2m
        ...
        // Add parent groups at the beginning (after UNASSIGNED if present)
        const parentGroupsList = Array.from(parentGroups.values());
        groups.unshift(...parentGroupsList);
